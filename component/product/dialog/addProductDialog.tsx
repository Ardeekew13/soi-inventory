...
const AddProductModal = (props: ProductModalProps) => {
	...
	useEffect(() => {
		if (record?.ingredientsUsed) {
			console.log('Product ingredientsUsed:', record.ingredientsUsed);
				...
				(ing: ProductIngredient) => {
					...
					return {
						item: {
							...
							name: ing.item.name,
							unit: ing.item.unit,
							pricePerUnit: ing.item.pricePerUnit || 0,
							isActive: (ing.item as any).isActive,
						},
		...
		if (existingIndex !== -1) {
			...
			updatedIngredients[existingIndex] = {
				...
				quantityUsed: parseFloat(
					(updatedIngredients[existingIndex].quantityUsed + 0.01).toFixed(4),
				),
			};
	...
	const handleDecrement = (id: string) => {
		const updatedIngredients = ingredients.map((ingredient) => {
			if (ingredient.item._id === id) {
				return {
					...ingredient,
					quantityUsed: Math.max(
						0.001,
						parseFloat((ingredient.quantityUsed - 0.01).toFixed(4)),
					),
				};
			}
			return ingredient;
	...
	const handleIncrement = (id: string) => {
		const updatedIngredients = ingredients.map((ingredient) => {
			if (ingredient.item._id === id) {
				return {
					...ingredient,
					quantityUsed: parseFloat((ingredient.quantityUsed + 0.01).toFixed(4)),
				};
			}
			return ingredient;
		});
	...
	const columns: TableProps<IngredientWithQuantity>["columns"] = [
			...
			render: (_, record: IngredientWithQuantity) => (
				<Space>
					<Button
						...
						disabled={record.quantityUsed <= 0.001}
					>
						-
					</Button>
					<InputNumber
						...
						min={0.001}
						step={0.001}
						formatter={(value) => `${parseFloat(value?.toString() || '0')}`}
		...
		},
		{
			...
			align: "right",
			render: (_, record: IngredientWithQuantity) => {
				const price = record.item.pricePerUnit || 0;
				const total = record.quantityUsed * price;
